...
Sim3ResidualStruct Sim3Tracker::calcSim3WeightsAndResidualSSE(const Sim3& referenceToFrame)
{
  ...
  const __m128 depthVarFacs = _mm_set1_ps((float)settings.var_weight);  // float depthVarFac = var_weight;	// the depth
                                                                        // var is over-confident. this is a constant
                                                                        // multiplier to remedy that.... HACK
  ...
  for (int i = 0; i < buf_warped_size - 3; i += 4)
  {
    ...
    __m128 depthValid = _mm_cmplt_ps(
        zeros, _mm_load_ps(buf_warped_idepthVar + i));  // bitmask 0xFFFFFFFF for idepth valid, 0x000000 otherwise
